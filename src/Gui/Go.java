 ...
 * @author User
 */
public class Go extends BaseForm {

    public Go(Resources res) {
        ...
        Tabs swipe = new Tabs();
//        Label[] spacers = new Label[lp.size()];
        if (lp != null) {
            for (int i = 0; i < lp.size(); i++) {
                addTab1(swipe,lp.get(i));
            }
        }
...
//        for (int i = 0; i < spacers.length-1; i++) {
//            Component.setSameSize(radioContainer, spacers[i], spacers[i+1]);
//            }
        
        add(LayeredLayout.encloseIn(swipe, radioContainer));

        SpanLabel lNom = new SpanLabel("Enjoy the best\nHunting & Fishing experience.");
    ...
    private void addTab(Tabs swipe, Image img, String likesStr, String commentsStr, String text) {
    ...
    private void addTab1(Tabs swipe, Publicity mdt) {
        ...
        Label comments = new Label(mdt.getCompagnie(), "CommandFocus");
        FontImage.setMaterialIcon(comments, FontImage.MATERIAL_CHAT);
        ScaleImageLabel image = new ScaleImageLabel(img);
        image.setUIID("Container");
